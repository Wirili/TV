...
export function urljoin(base, url) {
  ...
  if ((!u.path() || u.path().trim().length === 0) && baseU.path()) u.path(baseU.path());
...
export function req(url, obj) {
  try {
    const rsp = fetch(url, obj);
    return { headers: rsp.headers, content: rsp.text() };
  } catch (e) {
    console.log("req 调用出错！")
...
const local = {
  set: function (RKey, key, value) {},
  get: function (RKey, key) {
    return "";
  },
  delete: function (RKey, key) {},
};
